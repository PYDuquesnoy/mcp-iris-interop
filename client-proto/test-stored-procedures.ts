...
async function testStoredProcedures() {
  ...
  const spClassPath = path.join(__dirname, '..', '..', 'iris-samples', 'Test.StoredProc.cls');
  const spContent = fs.readFileSync(spClassPath, 'utf8');
  ...
  try {
    const result3 = await client.executeQuery(
      "SELECT Test.SimpleTest('Hello World') AS Result",
      [],
      config.namespace
    );
  ...
  try {
    const result6 = await client.executeQuery(
      "{call Test.SimpleTest(?)}",
      ['Hello World'],
      config.namespace
    );
  ...
  try {
    const result7 = await client.executeQuery(
      "{ ? = call Test.SimpleTest(?) }",
      ['Hello World'],
      config.namespace
    );

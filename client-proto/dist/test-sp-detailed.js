...
async function testStoredProcedures() {
    ...
    await runQuery('Simple SELECT', 'SELECT 1 AS One, 2 AS Two');
    await runQuery('Function with parameter', 'SELECT $LENGTH(?) AS StringLength', ['Testing 123']);
    await runQuery('Multiple parameters', 'SELECT ? AS First, ? AS Second, $LENGTH(?) AS Length', ['Hello', 'World', 'Test String']);
    await runQuery('Query system tables', 'SELECT TOP 3 ClassName, SqlName FROM %Dictionary.CompiledMethod WHERE SqlProc = 1 AND SqlName IS NOT NULL ORDER BY ClassName');
    // Now let's test CALL syntax with a known system procedure
    await runQuery('CALL syntax test', 'CALL %Library.GlobalEdit_Exists(?)', ['^test']);
    // Test with different CALL variations
    await runQuery('JDBC style CALL', '{call %Library.GlobalEdit_Exists(?)}', ['^test']);
    console.log('\n✅ Testing complete!');
}
// Run the tests

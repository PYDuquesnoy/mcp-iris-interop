...
async function testStoredProcedures() {
    ...
    try {
        const result1 = await client.executeQuery("SELECT $LENGTH(?) AS Length", ['Hello World'], config.namespace);
    ...
    try {
        const result2 = await client.executeQuery("SELECT $LENGTH(?) AS Length, $UPPER(?) AS Upper", ['Hello', 'world'], config.namespace);
    ...
    try {
        const result3 = await client.executeQuery("SELECT TOP 5 SqlName FROM %Dictionary.MethodDefinition WHERE SqlProc = 1", [], config.namespace);
    ...
    try {
        const result4 = await client.executeQuery("SELECT %SYSTEM_SQL.TOCHAR(CURRENT_TIMESTAMP, 'YYYY-MM-DD') AS Today", [], config.namespace);
    ...
    try {
        const result5 = await client.executeQuery("SELECT 1+1 AS Result, ? AS Input", ['Test Input'], config.namespace);

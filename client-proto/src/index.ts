...
program
  ...
  .action(async (target, requestClass, options) => {
    ...
    if (options.data) verboseLog(`Data: ${options.data}`, options.verbose);
...
program
  ...
  .action(async (options) => {
    ...
    if (options.sessionId) verboseLog(`Session ID: ${options.sessionId}`, options.verbose);
    if (options.sinceTime) verboseLog(`Since Time: ${options.sinceTime}`, options.verbose);
    
    try {
      const result = await client.exportEventLog(
        parseInt(options.maxEntries), 
        options.sessionId, 
        options.sinceTime
      );
      ...
      if (result.success === 1) {
        ...
        if (options.verbose && result.logEntries && result.logEntries.length > 0) {
          console.log('\nEvent Log Entries:');
          result.logEntries.forEach((entry: any, index: number) => {
...
program
  ...
  .action(async (options) => {
    ...
    try {
      const result = await client.exportMessageTrace(
        parseInt(options.maxEntries), 
        options.sessionId, 
        options.sinceTime,
        options.logEntries
      ...
      if (result.success === 1) {
        ...
        if (options.verbose && result.messageEntries && result.messageEntries.length > 0) {
          ...
          result.messageEntries.forEach((entry: any, index: number) => {
...
program
  ...
  .action(async (options) => {
    ...
    if (options.headerId) verboseLog(`Header ID: ${options.headerId}`, options.verbose);
    if (options.bodyClass) verboseLog(`Body Class: ${options.bodyClass}`, options.verbose);
    if (options.bodyId) verboseLog(`Body ID: ${options.bodyId}`, options.verbose);
    verboseLog(`Format: ${options.format}`, options.verbose);
    
    try {
      const result = await client.getMessageBody(
        options.headerId ? parseInt(options.headerId) : undefined,
        options.bodyClass,
        options.bodyId,
        options.format
      );
